...
# makes available that you can use the pygame.time.Clock() function with
...
def draw_grid():
    ...
    for i in range(block_number):
        ...
        pygame.draw.line(display, black, (grid_x, 0), (grid_x, total))
        # draws the horizontal line
        pygame.draw.line(display, black, (0, grid_y), (total, grid_y))
...
def draw_score():
    ...
    # speed_text = font.render(str(speed - (score * 2)), True, white)
    # makes a variable that uses the font to write the speed
    # display.blit(speed_text, [block_number -5, 0])
    # puts the speed next to the score
...
def color_generator():
    ...
    color = (red_num, green_num, blue_num)
    return color
...
class conditions:
    def reset(self):
        ...
        snake.body = [vector2(15, 15), vector2(14, 15), vector2(13, 15)]
        ...
        # speed = 150
        # sets the difficulty speed back to normal
    ...
    def fail(self):
        if snake.body[0].x < 0 or snake.body[0].x >= block_number:
            ...
            self.reset()
            # calls the reset function to reset everything
    ...
    def update(self):
        ...
        self.collision()
        self.fail()
        # checks all of these and updates them whenever update is called
...
# how many milliseconds inbetween each update
pygame.time.set_timer(WINDOW_UPDATE, speed)
# https://www.youtube.com/watch?v=QFvqStqPCRU&ab_channel=ClearCode
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == WINDOW_UPDATE:
            conditions.update()
        # if window_update is detected it will update all conditions
